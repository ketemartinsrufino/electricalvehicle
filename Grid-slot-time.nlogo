...
to setup
  ...
  ;carga max por tipo de carro [ P   M   G ]
  set carga-max-por-tipo-carro [100 200 300]
  ...
  [
    ...
    set shape "car"
    set size 3
    set carga-max item random 3 carga-max-por-tipo-carro
...
to go
  ...
  ifelse carro-pode-sair
   [
    ...
    [
      set localização "fora"
      set hora-saida hora
      set qte-slots-necessarios (hora-saida - hora-chegada) / slot-tempo
      hide-turtle
    ]
  ]
  [
    ...
    [
      ...
      set qte-slots-necessarios abs (hora-saida - hora-chegada) / slot-tempo
      if qte-slots-necessarios > 0 and qte-slots-necessarios < 1
      [
        set qte-slots-necessarios 1
      ]
      show-turtle
    ]
  ]
  ...
  [
    ...
    show "qte-slots-necessarios"
    show qte-slots-necessarios

    ifelse precisa-carregar and qte-slots-necessarios != 0
    [
      set carga-armazenada carga-armazenada + diferenca
      set color blue
      set label carga-armazenada
      set qte-slots-necessarios qte-slots-necessarios - 1
    ...
    if qte-slots-necessarios = 0 and estado = "desligado" [
      let transformador-mais-próximo min-one-of transformadores [distance myself]
      ask transformador-mais-próximo [
        ;set carga-atual carga-atual - diferenca
      ]
